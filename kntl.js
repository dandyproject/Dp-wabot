    ...
    create('Imperial', serverOption)
        .then((client) => {
            ...
            client.onAddedToGroup((chat) => {
                client.sendText(chat.groupMetadata.id, `Hae Semua *${chat.contact.name}* terimakasih sudah menginvite DP WhatsApp Bot ini, untuk melihat menu silahkan kirim *!help*`)
            })
            // listening on Incoming Call
            client.onIncomingCall((call) => {
                client.sendText(call.peerJid, 'Udah di Bilang Jangan Nelpon.Nelpon=Auto Block!!!')
...
`
        ...
        case 'ytmp3':
            ...
            break
        case 'tiktod':
            if(args.length !== 1) return client.reply(from, 'Maaf, link yang kamu kirim tidak valid', message.id)
        ...
        case 'quotemaker':
            arg = body.trim().split('|')
            if (arg.length >= 3) {
                client.sendText(from, 'sedang proses . . .') 
                const quotes = arg[1]
                const author = arg[2]
                const theme = arg[3]
